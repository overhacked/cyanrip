...
| -J                   | Only generate and print a CUE sheet, without ripping. Incompatible with -I                  |
|                      | **Misc. options**                                                                           |
...
All key=value pair tags must be separated by `:`. For track tags, the syntax is `-t track_number=key=value:key=value`. You need to specify the -t argument separately for each track. To use `:` or `=` inside a value, escape it with a backslash: `-t "1=title=One\: Two"`.
